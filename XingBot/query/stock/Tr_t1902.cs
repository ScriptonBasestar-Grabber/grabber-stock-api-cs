...
{
    public class Tr_t1902
    {
        public static readonly InBlockQuery InBlock_t1902 = (resModel, query, dict) =>
        {
        ...
        public static readonly OutBlockQuery OutBlock_t1902 = (resModel, query, writer) =>
        {
            var szTrCode = resModel.Name;
            ...
            {
                var result = new _t1902OutBlock()
                {
                    time = query.GetFieldData(block.Name, "shcode", i),
            ...
            writer.WriteHeader<_t1902OutBlock1>();
            for (var i = 0; i < query.GetBlockCount(block1.Name); i++)
            {
                var result = new _t1902OutBlock1()
                {
