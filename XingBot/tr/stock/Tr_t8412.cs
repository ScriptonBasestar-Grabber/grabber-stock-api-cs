...
{
    public class Tr_t8412
    {
        public static readonly InBlockQuery InBlock_t8412 = (resModel, query, dict) =>
        {
        ...
        public static readonly OutBlockQuery OutBlock_t8412 = (resModel, query, writer) =>
        {
            var szTrCode = resModel.Name;
            var block = resModel.Blocks[szTrCode + "OutBlock"];
            for (var i = 0; i < query.GetBlockCount(szTrCode + "OutBlock"); i++)
            {
                var result = new _t8412OutBlock()
                {
