...
{
    public class Tr_t1637
    {
        public static readonly InBlockQuery InBlock_t1637 = (resModel, query, dict) =>
        {
        ...
        public static readonly OutBlockQuery OutBlock_t1637 = (resModel, query, writer) =>
        {
            var szTrCode = resModel.Name;
            ...
            writer.WriteHeader<_t1637OutBlock1>();
            for (var i = 0; i < query.GetBlockCount(block1.Name); i++)
            {
                var result = new _t1637OutBlock1()
                {
