...
{
    public class Tr_t8411
    {
        public static readonly InBlockQuery InBlock_t8411 = (resModel, query, dict) =>
        {
        ...
        public static readonly OutBlockQuery OutBlock_t8411 = (resModel, query, writer) =>
        {
            var szTrCode = resModel.Name;
            var block = resModel.Blocks[szTrCode + "OutBlock"];
            for (var i = 0; i < query.GetBlockCount(szTrCode + "OutBlock"); i++)
            {
                var result = new _t8411OutBlock()
                {
            ...
            writer.WriteHeader<_t8411OutBlock1>();
            for (var i = 0; i < query.GetBlockCount(block.Name); i++)
            {
                var result = new _t8411OutBlock1()
                {
